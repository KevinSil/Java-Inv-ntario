...
public class produtoService {
    ...
    public produto patch(Long id, ProdutoUpdateRequest request) {
    produto produto = getById(id);
    if (request.getNome() != null)               produto.setNome(request.getNome());
    if (request.getPreco() != null)              produto.setPreco(request.getPreco());
    if (request.getFornecedor() != null)         produto.setFornecedor(request.getFornecedor());
    if (request.getQuantidadeStock() != null)    produto.setQuantidadeStock(request.getQuantidadeStock());
    if (request.getCategoria() != null)          produto.setCategoria(request.getCategoria());
    if (request.getEmPromocao() != null)         produto.setEmPromocao(request.getEmPromocao());
    if (request.getPercentagemPromocao() != null) produto.setPercentagemPromocao(request.getPercentagemPromocao());
    return repo.save(produto);
}
}
